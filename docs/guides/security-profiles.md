...
**1. Generate the CSMS certificate chain with the following cURL. `serverId` identifies which websocket server
config (its `id` in `config.json`). Passing it more than once (e.g.
`serverId=2&serverId=3`) regenerates the same chain for multiple servers at once:**

    curl --location 'http://localhost:8080/data/ocpprouter/certificateChain?tenant=1&serverId=2' \
...
**4. Set the password which will be stored in CitrineOS and used by EVerest when connecting with Security Profile 2:**
...
**5. Set variables for connection profiles. EVerest should have 2 connection configurations: one is Security Profile 1 (8081), 
...
**6. Restart EVerest. It will try to connect to CitrineOS using Security Profile 2. Check for the following logs to confirm
